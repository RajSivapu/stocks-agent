...
const routes = ["/", "/portfolio", "/ideas", "/companion", "/alerts", "/runs", "/runs/7d834dbd-75bb-4313-931f-09732f003932", "/system"];
...
test("hostile stored Telegram content remains inert and unsafe links remain text", async ({ page }) => {
  await page.goto("/alerts?fixture=hostile");
  await expect(page.getByText(/<img src=x onerror/)).toBeVisible();
  await expect(page.locator(".telegram-preview img")).toHaveCount(0);
  await expect(page.getByText("unsafe.example")).toBeVisible();
  await expect(page.getByRole("link", { name: "unsafe.example" })).toHaveCount(0);
  await expect(page.getByText(/no Telegram message was sent/i)).toBeVisible();
});
...
test("stale, owner-denied, and expired-session states disclose no private fixture data", async ({ page }) => {
  ...
  await page.goto("/?fixture=owner-denied");
