...
export function PortfolioPage({ data }: { data: PortfolioView }) {
  return (
    <div className="page-stack">
      ...
      <section className="section-block"><h2>Holdings</h2>{data.holdings.length === 0 ? <p className="empty-copy">No recorded holdings.</p> : <div className="table-scroll"><table><thead><tr><th>Ticker</th><th>Shares</th><th>Average</th><th>Price</th><th>Value</th><th>Risk levels</th></tr></thead><tbody>{data.holdings.map((item) => <tr key={item.ticker}><th>{item.ticker}<small>{item.bucket}</small></th><td>{item.shares}</td><td>{dollars(item.average_cost)}</td><td>{dollars(item.price)}<small>{item.freshness}</small></td><td>{dollars(item.value)}</td><td>Stop {dollars(item.stop)}<br />Target {dollars(item.target)}</td></tr>)}</tbody></table></div>}</section>
