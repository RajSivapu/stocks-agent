...
export function TodayPage({ data }: { data: TodayView }) {
  return (
    <div className="page-stack">
      ...
      <section className="section-block"><div className="section-heading"><h2>Portfolio at a glance</h2><Link to="/portfolio">Open portfolio</Link></div><div className="metric-grid"><div><span>Supported value</span><strong>{money(data.portfolio.value)}</strong></div><div><span>Cost basis</span><strong>{money(data.portfolio.cost_basis)}</strong></div><div><span>Unrealized</span><strong>{money(data.portfolio.unrealized_amount)}</strong></div></div><p className="muted">Portfolio prices {priceReceiptContext(data.portfolio.market_state, data.portfolio.data_as_of, data.portfolio.price_sources)}</p>{data.portfolio.value === null && <p className="inline-warning">Value is withheld until every required price has current receipt support.</p>}</section>
