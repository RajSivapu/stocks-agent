...
| Owner-only Personal Stock Agent Web v1 | Read-only API, direct-column database role, owner OTP auth, five primary responsive surfaces, receipt/staleness semantics, hostile-content isolation, private Site adapter, and browser/bundle verification | Live as private Site version 2 with API version 3; one-owner access, anonymous/non-owner denial, six owner GET routes, exact CORS/no-store, and source parity verified |
| Personal Stock Agent V1 release gate | Reviewed implementation binds the scheduled run UUID, deterministically records reports, validates report chains, restores the prior gateway on failed initial deployment, and strictly reconciles all 15 typed gates | Implementation `688d473` merged to `main` as `6819052`; CI, review, migrations, dry-run, rollback, functions, Site, source parity, and canaries passed. C6 awaits only the next existing post-merge scheduled V1 receipt chain |
