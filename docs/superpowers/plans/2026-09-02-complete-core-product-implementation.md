...
Local status: all automated tests, type/lint/build, dependency audits, SQL lint, secret scan,
legacy-migration rollback, and fresh-schema catalog parity pass. Live staging, Gate A-G evidence,
restore, rollback, owner soak, and friend-cycle commands remain unchecked.
