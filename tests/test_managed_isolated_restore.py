...
def test_actual_migration_retry_accepts_truthful_baseline_without_writing_historical_rows(tmp_path, monkeypatch):
    ...
    reference_transfer_capacity = next(
        item for item in deploy.candidate_migration_manifest()
        if item["path"] == "sql/migrations/20261025_reference_transfer_capacity.sql"
    )
    reference_finalization_set_based = next(
        item for item in deploy.candidate_migration_manifest()
    ...
    unresolved_packet_readback = next(
        item for item in deploy.candidate_migration_manifest()
