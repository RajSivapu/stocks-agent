...
export function planPreviewText(command) {
  ...
  return `Preview — record monthly ${command.ticker} reminder for $${number(command.amount)}; next due ${command.next_due_on} in core.\nThis records a reminder only; it does not schedule or place a brokerage purchase.`;
}
...
export function planResultText(result) {
  ...
  return `Recorded monthly ${result.ticker} reminder for $${number(result.amount)}; next due ${result.next_due_on} in ${result.bucket}. No brokerage purchase was scheduled or placed.`;
}
