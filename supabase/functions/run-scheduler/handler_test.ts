...
class FakeRepository implements SchedulerRepository {
  ...
  publishHoliday() {
    this.calls.push("holiday");
    return Promise.resolve({ status: "ready" });
  }
  runMaintenance() {
    this.calls.push("maintain");
    return Promise.resolve({ missed_slots: 0 });
  }
  claimPublications() {
    this.calls.push("claim-publications");
    return Promise.resolve([]);
  }
  finishPublication() {
    this.calls.push("finish-publication");
    return Promise.resolve({ status: "delivered" });
  }
  claimOperationalAlerts() {
    this.calls.push("claim-alerts");
    return Promise.resolve([]);
  }
  finishOperationalAlert() {
    this.calls.push("finish-alert");
    return Promise.resolve({ status: "delivered" });
  }
  readDueDecisions() {
    this.calls.push("read-due");
    return Promise.resolve([]);
  }
  applyOutcomeGrades() {
    this.calls.push("apply-grades");
...
Deno.test("scheduler authenticates before parsing and returns no secret material", async () => {
  ...
  const handler = createRunSchedulerHandler({
    repository,
    schedulerSecret: SECRET,
    telegramToken: "12345:opaque-token",
  });
