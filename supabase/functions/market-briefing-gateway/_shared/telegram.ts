...
const ALERT_CALLBACK = /^al:(arm|dismiss|pause|resume|ack|snooze20m|snooze1d):[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}:[1-9]\d{0,6}$/i;
...
function validAlertMarkup(markup: TelegramAlertInput["reply_markup"]): boolean {
  if (!Array.isArray(markup?.inline_keyboard) || markup.inline_keyboard.length !== 1) return false;
  const buttons = markup.inline_keyboard[0];
  return Array.isArray(buttons) && buttons.length >= 1 && buttons.length <= 3 &&
    buttons.every((button) =>
      button && typeof button.text === "string" && button.text.length >= 1 && button.text.length <= 40 &&
      !/\b(?:buy|sell|order)\b/i.test(button.text) && typeof button.callback_data === "string" &&
      button.callback_data.length <= 64 && ALERT_CALLBACK.test(button.callback_data)
    );
}
...
export async function sendTelegramAlert(
  ...
  if (!/^-?\d+$/.test(chatId) || token.length < 8 || token.length > 256 || /\s/.test(token) ||
    typeof alert?.body !== "string" || alert.body.length < 1 || alert.body.length > 3_500 ||
    !validAlertMarkup(alert.reply_markup)) {
    throw new TelegramDeliveryError("definitive", []);
  }
  const messageId = await sendPart(alert.body, chatId, token, fetchImpl, alert.reply_markup);
  const acceptedAt = now();
  if (Number.isNaN(acceptedAt.valueOf())) throw new TelegramDeliveryError("ambiguous", [messageId]);
  return {
    status: "accepted_by_telegram",
