import { isNyseHoliday, quoteAllowedForPhase } from "./market-calendar.ts";
...
Deno.test("NYSE holiday lookup is exact and weekends are not mislabeled holidays", () => {
  ...
  assert(!isNyseHoliday("2026-09-05", holidays), "weekend is not a holiday label");
});
...
Deno.test("intraday and regular-session on-demand quotes obey the same age bound", () => {
  ...
  assert(
    quoteAllowedForPhase("intraday", quote("2026-09-02T16:40:00.000Z", "REGULAR"), now, holidays, 20),
    "20-minute quote should pass",
  );
  assert(
    !quoteAllowedForPhase("intraday", quote("2026-09-02T16:39:00.000Z", "REGULAR"), now, holidays, 20),
    "21-minute quote should fail",
  );
  assert(
    !quoteAllowedForPhase("on-demand", quote("2026-09-02T16:39:00.000Z", "REGULAR"), now, holidays, 20),
...
Deno.test("outside-session on-demand requires the latest official close", () => {
  ...
  assert(
    quoteAllowedForPhase("on-demand", quote("2026-09-02T20:00:00.000Z", "CLOSED"), afterClose, holidays, 20),
    "same-day official close should pass after close",
  );
  assert(
    !quoteAllowedForPhase("on-demand", quote("2026-09-01T20:00:00.000Z", "CLOSED"), afterClose, holidays, 20),
