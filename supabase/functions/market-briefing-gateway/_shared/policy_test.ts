...
Deno.test("only approved fresh policy evaluations project inert alert drafts", () => {
  ...
  if (projected === null) {
    throw new Error("approved trigger did not project a draft");
  }
...
Deno.test("stale live quote downgrades Buy and on-demand cannot bypass it", () => {
  ...
  assert(
    onDemand.reason_codes.includes("QUOTE_STALE"),
    "on-demand bypassed freshness",
  );
});
...
Deno.test("outside-session research is conditional and cannot emit an entry trigger", () => {
  ...
  const closedContext = context({
    holding_quotes: {
      VTI: quote("VTI", "400", "2026-09-02T20:00:00.000Z", "CLOSED"),
    },
  });
  ...
  assert(
    conditional.reason_codes.includes("OUTSIDE_SESSION_CONDITIONAL"),
    "conditional metadata absent",
  );

  const trigger = evaluate(
    candidate({ phase: "on-demand", notification_kind: "entry_trigger" }),
    closedContext,
    close,
    config(),
    afterClose,
  ...
  assert(
    trigger.reason_codes.includes("QUOTE_SESSION_MISMATCH"),
    "outside entry trigger was not vetoed",
  );
});
...
Deno.test("normalized decisions preserve the verified provider market state", () => {
  const result = evaluate(
    candidate(),
    context(),
    quote("CENX", "47.02", "2026-09-02T16:55:00.000Z", "REGULAR"),
  );
...
Deno.test("calendar coverage and pre-market entry triggers fail closed", () => {
  ...
  assert(
    result.reason_codes.includes("CALENDAR_COVERAGE_MISSING"),
    "calendar gap absent",
  );
  ...
  const preContext = context({
    holding_quotes: {
      VTI: quote("VTI", "400", "2026-09-01T20:00:00.000Z", "CLOSED"),
    },
  });
  const preResult = evaluate(
    pre,
    preContext,
    quote("CENX", "47.02", "2026-09-01T20:00:00.000Z", "CLOSED"),
    config(),
    preNow,
  );
  assertEquals(preResult.final_action, null);
  assert(
    preResult.reason_codes.includes("QUOTE_SESSION_MISMATCH"),
    "pre-market trigger was not vetoed",
  );
});
...
Deno.test("Analyst and Checker completion and verdicts remain distinct", () => {
  ...
  assert(
    analyst.reason_codes.includes("ANALYST_INCOMPLETE"),
    "analyst reason absent",
  );
  const checker = evaluate(
    candidate({ checker: { ...candidate().checker, completed: false } }),
  );
  assert(
    checker.reason_codes.includes("CHECKER_INCOMPLETE"),
    "checker reason absent",
  );
  ...
  assert(
    downgraded.reason_codes.includes("CHECKER_DOWNGRADE"),
    "downgrade reason absent",
  );
...
Deno.test("candidate without approved exposure evidence is downgraded", () => {
  const coMention = candidate({
    relationship_type: "direct",
    evidence: [{ ...candidate().evidence[0], exposure_kind: null }],
  });
  const result = evaluate(coMention);
  assertEquals(result.final_action, "watch");
  assert(
    result.reason_codes.includes("EXPOSURE_EVIDENCE_MISSING"),
    "exposure veto absent",
  );
});

Deno.test("missing or copied Checker provenance is vetoed", () => {
  const missing = evaluate(candidate({
    relationship_type: "direct",
    checker: { ...candidate().checker, completed: false },
  }));
  assertEquals(missing.final_action, null);
  const copied = evaluate(candidate({
    relationship_type: "direct",
    checker: { ...candidate().checker, reason: candidate().analyst.reason },
  }));
  assertEquals(copied.final_action, null);
  assert(
    copied.reason_codes.includes("CHECKER_COPIED"),
    "copied checker passed",
  );
});
...
Deno.test("ownership and sell quantity mismatches veto", () => {
  ...
  for (const action of ["add", "reduce", "sell"] as Action[]) {
    const c = candidate({
      action,
      analyst: { ...candidate().analyst, action },
    });
    assertEquals(evaluate(c).final_action, null);
  }
  const sell = candidate({
    action: "sell",
    analyst: { ...candidate().analyst, action: "sell" },
    proposed_amount: null,
    proposed_shares: "21",
  });
  const sold = evaluate(
...
Deno.test("long price relationships are strict", () => {
  for (
    ...
    assert(
      result.reason_codes.includes("PRICE_RELATION_INVALID"),
      "invalid levels passed",
    );
  }
});
...
Deno.test("fractional shares reconcile to amount within one cent", () => {
  ...
  const result = evaluate(
    candidate({ proposed_amount: "2057.04", proposed_shares: "43.748192" }),
    ctx,
  );
  assertEquals(result.final_action, "buy");
  assert(
    !result.reason_codes.includes("AMOUNT_SHARES_MISMATCH"),
    "cent reconciliation failed",
  );
});
...
Deno.test("risk, reward-risk, and position limits only downgrade Buy", () => {
  ...
  assert(
    highRisk.reason_codes.includes("TRADE_RISK_EXCEEDED"),
    "risk cap absent",
  );
  const lowReward = evaluate(candidate({ target: "50" }));
  assert(
    lowReward.reason_codes.includes("REWARD_RISK_TOO_LOW"),
    "reward-risk floor absent",
  );
  const oversized = evaluate(
    candidate({ proposed_amount: "4702", proposed_shares: "100" }),
  );
  assert(
    oversized.reason_codes.includes("POSITION_CAP_EXCEEDED"),
    "position cap absent",
  );
...
Deno.test("Add uses the recorded holding stop and cannot substitute a model level", () => {
  ...
  const add = candidate({
    action: "add",
    analyst: { ...candidate().analyst, action: "add" },
  });
  const result = evaluate(add, ctx);
  assertEquals(result.final_action, "watch");
  assert(
    result.reason_codes.includes("STOP_REQUIRED"),
    "missing authoritative Add stop passed",
  );
});
...
Deno.test("portfolio completeness and loss locks block Buy", () => {
  ...
  assert(
    missing.reason_codes.includes("PORTFOLIO_VALUE_INCOMPLETE"),
    "missing holding quote passed",
  );
  const uncovered = evaluate(
    candidate(),
    context({ portfolio_command_coverage_complete: false }),
  );
  assert(
    uncovered.reason_codes.includes("PORTFOLIO_VALUE_INCOMPLETE"),
    "incomplete ledger passed",
  );
  const daily = evaluate(candidate(), context({ realized_pnl_today: "-1300" }));
  assert(
    daily.reason_codes.includes("DAILY_LOSS_LOCKOUT"),
    "daily loss lock absent",
  );
  const consecutive = evaluate(
    candidate(),
    context({ consecutive_completed_losses: 3 }),
  );
  assert(
    consecutive.reason_codes.includes("CONSECUTIVE_LOSS_LOCKOUT"),
    "loss streak lock absent",
  );
});
...
Deno.test("speculative learning threshold blocks real-money action", () => {
  ...
  assertEquals(result.final_action, "watch");
  assert(
    result.reason_codes.includes("SPECULATIVE_LEARNING_ONLY"),
    "speculative gate absent",
  );
});
...
Deno.test("matching VTI owner plan skips stops but keeps core allocation checks", () => {
  ...
  const ctx = context({
    holdings: [voo],
    holding_quotes: { VOO: quote("VOO", "500") },
    owner_plans: [plan],
  });
  const c = candidate({
    ticker: "VTI",
  ...
  assert(
    !result.reason_codes.includes("STOP_REQUIRED"),
    "plan incorrectly required a stop",
  );
  assert(
    !result.reason_codes.includes("REWARD_RISK_TOO_LOW"),
    "plan incorrectly required reward-risk",
  );

  const bad = evaluate(
    { ...c, proposed_amount: "301" },
    ctx,
    quote("VTI", "400"),
  );
  assertEquals(bad.final_action, null);
  assert(
    bad.reason_codes.includes("OWNER_PLAN_MISMATCH"),
    "bad plan identity passed",
  );
});
...
Deno.test("holding alert edges require thresholds, freshness, and inactive flags", () => {
  ...
  const alert = (notification_kind: DecisionCandidate["notification_kind"]) =>
    candidate({
      action: "hold",
      notification_kind,
      proposed_amount: null,
      ...
      analyst: {
        ...
        action: "hold",
        confidence: "medium",
        reason: "Threshold edge.",
      },
    });
  for (
    const [kind, price, flag] of [
    ...
    assert(
      result.holding_state_change?.[flag] === true,
      `${kind} did not set its edge flag`,
    );
  }
  const active = context({
    holdings: [{ ...owned, stop_near_alert_active: true }],
    holding_quotes: { CENX: quote("CENX", "46") },
  });
  const duplicate = evaluate(alert("stop_near"), active, quote("CENX", "46"));
  assertEquals(duplicate.final_action, null);
  assert(
    duplicate.reason_codes.includes("ALERT_ALREADY_ACTIVE"),
    "duplicate alert passed",
  );
});
...
Deno.test("hold override suppresses mechanical stop but not evidenced thesis break", () => {
  const owned = holding({
    ...
    bucket: "growth",
    stop: "45",
    hold_override_until: "2026-09-05",
  });
  const ctx = context({
    holdings: [owned],
    holding_quotes: { CENX: quote("CENX", "44") },
  ...
  const thesis = {
    ...stop,
    notification_kind: "thesis_break" as const,
...
Deno.test("holding output contains only server-authorized high-water and edge fields", () => {
  ...
  const ctx = context({
    holdings: [owned],
    holding_quotes: { CENX: quote("CENX", "49") },
  });
  const c = candidate({
    action: "hold",
    proposed_amount: null,
  ...
  assertEquals(Object.keys(change!).sort(), [
    "high_water_price",
    "stop_alert_active",
  ...
  const staleChange =
    evaluate(c, ctx, quote("CENX", "49", "2026-09-02T16:39:00.000Z"))
...
Deno.test("increasing shares or widening stop cannot upgrade a failed proposal", () => {
  const failed = evaluate(
    candidate({ proposed_amount: "9404", proposed_shares: "200" }),
  );
  const larger = evaluate(
    candidate({ proposed_amount: "18808", proposed_shares: "400" }),
  );
  const wider = evaluate(
    candidate({ proposed_amount: "9404", proposed_shares: "200", stop: "1" }),
  );
